...
### Fixed
- AMD static polling now reads nested PPT power limit fields from newer `amd-smi` output instead of falling back to a bogus `1W` display ceiling.

## [0.2.2] - 2026-05-17

### Fixed
- AMD metric polling now handles `amd-smi` JSON entries where unsupported GPUs report whole metric blocks such as `usage` as `"N/A"` instead of objects.

## [0.2.1] - 2025-12-26

### Fixed
- Package now includes CSS stylesheet files (`.tcss`) - fixes crash on pip-installed versions
